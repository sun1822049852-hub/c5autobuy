...
- [x] **Step 1: 写失败测试，断言 builder config 的 `extraResources` 不再包含 `.venv`，而是包含 `python_deps`**
- [x] **Step 2: 写失败测试，断言 preflight 会准备最小 Python 依赖资源，而不是校验 `resources/.venv`**
- [x] **Step 3: 运行 focused packaging tests，确认红灯**
...
- [x] **Step 1: 写失败测试，断言 packaged embedded startup 会先调用 `ensureManagedPythonRuntime`，再把返回的托管 `python.exe` 传给 `startPythonBackend`**
- [x] **Step 2: 写失败测试，断言 runtime bootstrap 失败时不会调用 backend 启动，并落到 failure window**
- [x] **Step 3: 运行 focused packaging tests，确认红灯**
...
- [x] **Step 1: 写失败测试，断言 manifest 与 `python.exe` 都完整时直接复用，不触发下载**
- [x] **Step 2: 写失败测试，断言缺失 runtime 时会下载 zip、校验 SHA256、解压 staging、安装 `python_deps` 并写 manifest**
- [x] **Step 3: 写失败测试，断言 SHA256 不匹配或解压失败时会清理临时文件 / staging 并抛出可展示错误**
- [x] **Step 4: 运行 focused bootstrap tests，确认红灯**
...
- [x] **Step 1: 写最小配置模块，固定 Python embeddable 版本、官方 URL、SHA256、托管目录名与依赖裁剪排除名单**
- [x] **Step 2: 写最小资源导出实现，把开发 `.venv/Lib/site-packages` 裁剪复制到 `app_desktop_web/build/python_deps/Lib/site-packages`**
- [x] **Step 3: 在 preflight 中先执行 renderer build，再导出 `python_deps`，并用开发 `.venv` 对导出结果做一次 import smoke check**
- [x] **Step 4: 回跑 packaging tests，确认 Task 1 转绿**

Run: `npm --prefix app_desktop_web test -- tests/electron/program_access_packaging.test.js --run`

Expected: PASS，builder config / preflight 契约转绿。

- [x] **Step 5: Commit（阶段提交点已并入最终收口 commit）**
...
- [x] **Step 1: 移除 `.venv` extraResources**
- [x] **Step 2: 增加 `build/python_deps` extraResources**
- [x] **Step 3: 保持 `app_backend`、`xsign.py`、release client config 打包路径不变**
- [x] **Step 4: 回跑 packaging tests，确认 builder config 契约稳定**
...
- [x] **Step 1: 写最小路径解析与 manifest 读取逻辑**
- [x] **Step 2: 写下载 + SHA256 校验 + staging 解压逻辑**
- [x] **Step 3: 把 packaged `python_deps` 安装到托管 runtime 的 `Lib/site-packages`，并修补 embeddable `._pth` 以启用 site-packages**
- [x] **Step 4: 写 manifest 完成态与损坏重试逻辑**
- [x] **Step 5: 回跑 bootstrap tests，确认 Task 3 转绿**
...
- [x] **Step 1: packaged release 启动前先调用 `ensureManagedPythonRuntime()`，只在成功后启动 backend**
- [x] **Step 2: 开发态继续保留 `.venv` 祖先回溯解析，不引入下载链**
- [x] **Step 3: packaged 失败路径维持 fail-closed，并沿用现有 failure window 文案通道**
- [x] **Step 4: 回跑 packaging + python backend tests，确认 Task 2 与开发态契约转绿**
...
- [x] **Step 1: 跑 focused Electron tests**

Run: `npm --prefix app_desktop_web test -- tests/electron/program_access_packaging.test.js tests/electron/python_backend.test.js tests/electron/python_runtime_bootstrap.test.js --run`

- [x] **Step 2: 跑 `pack:win` 生成新的 `win-unpacked`**

Run: `npm --prefix app_desktop_web run pack:win`

Expected: PASS，且 `app_desktop_web/release/win-unpacked/resources/.venv` 不存在。

- [x] **Step 3: 记录新 `win-unpacked` 与 installer 体积，和旧基线 `1.02GB / 282.77MB` 对比**

Run: `Get-ChildItem app_desktop_web/release -Force`

- [x] **Step 4: 若环境允许，再跑 installer 构建**

Run: `npm --prefix app_desktop_web run build:win`

- [x] **Step 5: 做 packaged smoke**

Check:
- 清空托管 Python runtime 后首次启动，确认 loading -> 下载 -> backend ready -> app
- 断网启动，确认阻断错误与重试入口

- [x] **Step 6: 更新 session log，写清体积变化、实现范围、验证结果与剩余风险**
- [x] **Step 7: 只有形成新的稳定规则时才更新 memory**
- [x] **Step 8: README 已核对，本轮无需改动打包 / 首启说明**
- [x] **Step 9: 按真实执行进度勾选计划**
