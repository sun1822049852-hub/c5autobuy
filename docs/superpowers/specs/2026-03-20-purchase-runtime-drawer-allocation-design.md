...
- `api高速查询器`
  ...
  - 单账号具备 `api_key` 时可用
  - 查询频率心智为 `1s/5次`
  - 成功结果需要先做本地处理，再进入购买分配池
- `浏览器查询器`
  - 对应 `token`
  - 单账号具备有效 `token/cookie` 时可用
  - 作为浏览器链路查询能力

实现层继续以 `new_api / fast_api / token` 为内部枚举真相源；
文档、UI 文案、后续讨论允许使用：
...
查询能力失效必须按“能力粒度”传播，而不是默认整号全灭。

查询侧产品语义冻结为：

- 支持单独禁用某个查询器
- 不把“整账号查询总开关”作为本轮需求写入设计
- 购买侧 `purchase_disabled` 仍然只属于购买能力控制，不属于查询器分配语义

当前代码里虽然存在 `Account.disabled` 字段，但在本轮设计中：

- 不把它当作查询侧产品能力真相源
- 不把“整号禁用 -> 三类查询器全灭”写成需求
- 查询侧能力控制以单查询器开关为准：
  - `new_api_enabled`
  - `fast_api_enabled`
  - `token_enabled`
...
- 若 `api_key` 失效：
  - `api查询器(new_api)` 出池
  - `api高速查询器(fast_api)` 出池
  - `浏览器查询器(token)` 不受影响
- 若 `token/cookie` 失效：
  - 只让 `浏览器查询器(token)` 出池
  - `api查询器(new_api)` 与 `api高速查询器(fast_api)` 不受影响
- 若用户手动禁用某个查询器：
  - 只让对应查询器出池
  - 其他查询器保持原状
...
- `api_key` 恢复后，只恢复 `new_api + fast_api`
- `token/cookie` 恢复后，只恢复 `token`
- 某个查询器被手动重新启用后，只恢复该查询器

分配器和商品状态必须跟随这套规则自然变化：

- 某类查询器出池后，只影响该 mode 下的可用 worker 数
- 商品是否继续保持专属态、退入共享池、或显示无可用账号，仍由对应 allocator 根据最新可用 worker 数自行 reconcile
- 不允许把单一能力失效错误放大成整账号下所有查询能力同时失效
...
- 购买侧“每账号 3 个小购买池”在本 spec 中只作为后续购买运行时设计参考
- 本轮不据此扩写新的购买调度实现
- 当前热应用设计也不依赖这套购买侧内部结构
