...
> 状态更新（2026-04-19）：
> 本文档在“远端统一鉴权 / 会员服务端判定”层面仍可作为参考，但对当前本地桌面主线而言已部分过期。
> 最新冻结边界见 `docs/superpowers/specs/2026-04-19-program-membership-shared-workspace-design.md`。
> 其中“本地业务表统一挂 `owner_user_id`、运行时目录按用户拆分、按程序账号切本地工作区”的设计，不再适用于当前本地共享工作区语义。
