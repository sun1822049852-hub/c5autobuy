...
- 切换配置时只更换商品配置、分配与统计账本，不关闭账号查询 session
- 账号查询 session（HTTP client / browser context / cookie 登录态）必须跨配置切换复用
- 快速去重必须参考 `autobuy.py`
- `autobuy.py` 的快速去重逻辑为：
  - 使用 `total_wear_sum`
  - 保留 12 位小数
...
- `账号 session`：
  - 指账号登录态、cookie、token、HTTP client、浏览器上下文
  - 这层在切换配置、等待购买账号恢复、恢复查询时都必须复用
- `runtime_session_id`：
  - 只是当前配置统计账本与旧结果判废用的运行代号
  - 切配置、重启、清空统计时必须更换
...
- 不关闭账号查询 session
- 只切换当前配置指针、商品分配和统计账本
- 旧结果按旧 `runtime_session_id` 判为 stale 并丢弃
